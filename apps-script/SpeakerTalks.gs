...
function saveSpeakerTalkSelection(speakerId, talkNumbers) {
  const speaker = getSpeaker(speakerId);
  ...
  if (sheet.getLastRow() > 1) {
    ...
    for (let index = rows.length - 1; index >= 0; index--) {
      if (String(rows[index][0]) === String(speakerId)) {
  ...
  logAction_('MISE_A_JOUR_DISCOURS', 'ORATEUR', speakerId, {
    nom: speaker.fullName,
    discours: uniqueNumbers
  });
