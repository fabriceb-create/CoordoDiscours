function getDashboardData() {
  return measureServerOperation_('getDashboardData', function () {
      const now = new Date();
      ...
      const speakers = listSpeakersWithCongregations_('', false, congregations);
      const allTalks = listTalks('', true);
      const activeTalks = allTalks.filter(function (item) { return item.active; });
      const allPlannings = listPlanningsWithResources_('', true, speakers, allTalks);
      ...
      const hospitalities = listHospitalitiesWithPlannings_('', allPlannings);
      const invitations = listInvitationsWithPlannings_('', allPlannings);
