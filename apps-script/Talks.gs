function listTalks(searchText, includeInactive) {
  ...
  return sheetRowsAsObjects_(sheet).map(function (row) {
    ...
    const metadata = getEntityVersion_('DISCOURS', number);
    return {
      number: number,
      ...
      versionUpdatedAt: metadata.updatedAt,
...
function saveTalk(payload) {
  ...
  try {
    ...
    if (existing) assertEntityVersion_('DISCOURS', number, payload.version);
...
function setTalkActive(number, active, expectedVersion) {
  assertAccess_('COORDINATEUR');
  ...
  const saved = saveTalk(Object.assign({}, before, { active: Boolean(active), version: expectedVersion || before.version }));
...
function importTalkReference(rows) {
  ...
  rows.forEach(function (row, index) {
    try {
      ...
      const existing = listTalks('', true).find(function (talk) { return talk.number === number; });
      saveTalk({ number: number, title: title, active: row.active !== false, version: existing ? existing.version : '' });
